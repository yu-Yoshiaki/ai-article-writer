...
export function useArticle(articleId?: string) {
  ...
  const { data: article, error, isLoading, mutate: mutateArticle } = useSWR<Article>(
  ...
  const handleThemeSubmit = useCallback(async (theme: string) => {
  ...
  const handleUpdateSectionTitle = useCallback((index: number, title: string) => {
    if (!article) return;
    const newSections = [...article.sections];
  ...
  const handleSelectVariation = useCallback((sectionIndex: number, variationIndex: number) => {
    if (!article) return;
    const newSections = [...article.sections];
  ...
  const handleRegenerateVariations = useCallback(async (sectionIndex: number) => {
    if (!article) return;
  ...
  const handleToneChange = useCallback(async (tone: ArticleTone) => {
    if (!article) return;
  ...
  const handleUpdateContent = useCallback(async (updatedArticle: Article) => {
    try {
